...
run_connection_test() {
  local addr="$1"
  local output_file
  output_file="$(mktemp /tmp/swing_instruction_connection.XXXXXX)"

  log "Running connection-only test before real action execution."
...
usage() {
  cat <<EOF
Usage: $(basename "$0") "中文指令" [--execute] [--addr MAC] [--model MODEL] [--no-sudo] [--skip-bluetooth-fix] [--bluetooth-device VID:PID] [--update-bluetooth-firmware] [--install-bluetooth-fix] [--no-bluetooth-retry]

Examples:
  $(basename "$0") "起飞后悬停2秒再降落"
  $(basename "$0") "起飞后悬停2秒再降落" --execute --addr E0:14:89:09:3D:CB

Default model:
  qwen3.5:4b

Real execution flow:
  Bluetooth fix -> scan Swing if --addr is omitted -> connection test -> action preview -> type 确认执行 -> pyparrot flight
EOF
}
...
main() {
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      *)
        passthrough+=("$1")
        if [ "$1" = "--save-actions" ] || [ "$1" = "--log-dir" ] || [ "$1" = "--retries" ]; then
  ...
  if [ "$execute" = "1" ]; then
    ...
    log "Preparing Bluetooth for real execution."
    ...
    if [ -n "$swing_addr" ]; then
      ...
      log "Scanning Swing address for real execution."
    ...
    run_connection_test "$swing_addr" || die "Connection test failed. Real action execution was not started."

    if [ "$addr_in_args" = "0" ]; then
      passthrough+=("--addr" "$swing_addr")
    fi

    run_privileged env PYTHONPATH="$PROJECT_DIR/src" SWING_LLM_MODEL="$SWING_LLM_MODEL" "$PYTHON_BIN" \
      -m swing_control.app.run_instruction \
      "${passthrough[@]}"
  else
