...
run_connection_test() {
  local addr="$1"
  local output_file
  output_file="$(mktemp /tmp/swing_interactive_connection.XXXXXX)"

  log "Running connection-only test before interactive execution."
...
usage() {
  cat <<EOF
Usage: $(basename "$0") [--execute] [--addr MAC] [--model MODEL] [--no-sudo] [--skip-bluetooth-fix] [--bluetooth-device VID:PID] [--update-bluetooth-firmware] [--install-bluetooth-fix] [--no-bluetooth-retry]
...
Interactive flow:
  type Chinese instruction -> action preview -> type 确认执行 when --execute -> pyparrot flight -> next instruction
EOF
}
...
main() {
  ...
  if [ "$execute" = "1" ]; then
    ...
    log "Preparing Bluetooth for interactive real execution."
    ...
    if [ -n "$swing_addr" ]; then
      ...
      log "Scanning Swing address for interactive execution."
    ...
    run_connection_test "$swing_addr" || die "Connection test failed. Interactive execution was not started."
    ...
    run_privileged env PYTHONPATH="$PROJECT_DIR/src" SWING_LLM_MODEL="$SWING_LLM_MODEL" "$PYTHON_BIN" \
      -m swing_control.app.interactive_control \
      "${passthrough[@]}"
  else
