...
set_param([modelName '/Scope XYZ'], 'NumInputPorts', '1');
...
if ~isempty(mach)
    ...
    if ~isempty(chart)
        ...
        limits = [mapData.limits.min_x, mapData.limits.max_x, ...
            mapData.limits.min_y, mapData.limits.max_y, ...
            mapData.limits.min_z, mapData.limits.max_z];
        zones = zeros(numel(mapData.no_fly_zones), 3);
        for zoneIndex = 1:numel(mapData.no_fly_zones)
            zone = mapData.no_fly_zones(zoneIndex);
            zones(zoneIndex, :) = [zone.center.x, zone.center.y, ...
                zone.radius_m + zone.buffer_m];
        end
        chart.Script = sprintf([ ...
            ...
            '        y < limits(3) || y > limits(4) || ...\n', ...
            '        z < limits(5) || z > limits(6)\n', ...
            '    safeFlag = 0; return;\n', ...
            'end\n', ...
            'for i = 1:size(zones, 1)\n', ...
            '    dist = hypot(x - zones(i, 1), y - zones(i, 2));\n', ...
            '    if dist <= zones(i, 3)\n', ...
            '        safeFlag = 0; return;\n', ...
            '    end\n', ...
            'end\n', ...
            'end\n' ...
        ], mat2str(limits, 17), mat2str(zones, 17));
    end
end
